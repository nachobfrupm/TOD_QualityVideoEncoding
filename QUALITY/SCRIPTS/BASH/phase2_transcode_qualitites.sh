...
ffmpeg -s ${INPUT_WIDTH}x${INPUT_HEIGHT} -framerate $FPS_ORIGINAL_VIDEO -y -i ${INPUT_FILE}.yuv -c:v libx264 -x264-params "nal-hrd=cbr" -b:v ${BIT_RATE}k -bufsize 10000k $MASTER_VIDEO_FILE
...
do    
    ...
    echo "BITRATE,VMAF,PSNR,SI,TI,P1204_3_MOS,VCA_E,VCA_H,EVCA_SC,EVCA_TC,COVER_SSC,COVER_TSC,COVER_ASC,COVER_FSC,CARS,INFERENCE_TIME_MS" > ${input_mp4_file}.csv
