...
do
    ...
    python detect.py --weights yolov7.pt --conf 0.25 --img-size 640 --source $input_mp4_file > ${input_mp4_file}.txt
...
# Process all average files
echo "BITRATE,FRAME,CARS,BICYCLES,MOTORCYCLES,BUSES,TRUCKS,TRAFFIC_LIGHTS,PERSONS,STOP_SIGNS,INFERENCE_TIME_MS" > ${input_dir}/final_data.csv
for input_average_file in $(ls ${input_dir}/*.average.csv)
    do  
        tail -1 $input_average_file >> ${input_dir}/final_data.csv
done

# Generate plot file

python3 plot_file.py ${input_dir}/final_data.csv "YOLOV7 Degradation"
